...
def calcCYK(w):
    ...
    # where m is increasing distance between i and j
    for m in range(L):
        for i in range(L):
            j = i+(m)
            ...
            if j<L:
                ...
                for k in range(j-i+1):
                    #iterating through all possible productions
                    for s in range(VarNum):
                        ...
                        if i==j:
                            ...
                            #iterating through
                            for num1 in range(len(first)):
                                for num2 in range(len(first)-num1):
                                    # print first[num1]
                                    # print second[num2]
                                    print 'here'
                                    print num1
                                    print num2
                                    if first[num1]==True and second[num1+num2]==True and existProd(s, num1, num1+num2):
                                        X[i][j][s] = True
                                        print True

                            # if existProd(s, first[1], second[1]):
                            #     X[i][j][s] = True
                            #     print True
                            # if existProd(s, first[1], second[2]):
                            #     X[i][j][s] = True
                            #     print True
                            # if existProd(s, first[2], second[2]):
                            #     X[i][j][s] = True
                            #     print True
    print existProd(0, 1, 2)
    global production
    print production
    return X
